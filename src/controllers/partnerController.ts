...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    const todayPickups = pickups.filter((p: any) => new Date(p.createdAt) >= todayStart).length;
    ...
    const acceptedPickups = pickups.filter((p: any) => p.status === 'Accepted').length;
    ...
    const totalEarnings = transactions.reduce((acc: number, curr: any) => acc + curr.amount, 0);
...
export const acceptPickup = async (req: Request, res: Response) => {
  try {
    ...
    pickup.partnerName = (req as any).user?.name || 'Partner';
    await pickup.save();
...
export const getCollectionInventory = async (req: Request, res: Response) => {
  try {
    const partnerId = (req as any).user?.id;
