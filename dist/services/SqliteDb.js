...
export const sqliteFindUserByEmail = async (email) => {
    const db = await getSqliteDb();
    return db.get('SELECT * FROM users WHERE email = ?', [email.toLowerCase()]);
};
export const sqliteFindUserByPhone = async (phone) => {
    const db = await getSqliteDb();
    return db.get('SELECT * FROM users WHERE phone = ?', [phone]);
};
export const sqliteCreateUser = async (data) => {
    const db = await getSqliteDb();
    await db.run(`INSERT INTO users (id, email, name, password, phone, google_id) VALUES (?, ?, ?, ?, ?, ?)`, [data.id, data.email.toLowerCase(), data.name, data.password || null, data.phone || null, data.googleId || null]);
    await db.run(`INSERT INTO user_profiles (id, name, email, phone) VALUES (?, ?, ?, ?)`, [data.id, data.name, data.email, data.phone || null]);
    await db.run(`INSERT INTO user_wallets (id, user_id, balance, eco_points, level) VALUES (?, ?, ?, ?, ?)`, [data.id, data.id, 0, 0, 1]);
    return sqliteFindUserByEmail(data.email);
};
export const sqliteGetUserProfile = async (userId) => {
    const db = await getSqliteDb();
