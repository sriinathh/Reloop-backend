...
const handleUpdateProfile = async (req, res) => {
    try {
        ...
        if (!useSqlite()) {
            ...
            if (ifsc_code)
                walletUpdate.ifscCode = ifsc_code;
            if (bank_name)
...
router.post('/kyc/verify', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        let backUrl = '';
        if (FrontBase64)
            frontUrl = await uploadToCloudinary(FrontBase64, 'kyc');
        if (BackBase64)
            backUrl = await uploadToCloudinary(BackBase64, 'kyc');
        const parsedKyc = {
            name: 'Srinath',
            dob: '15/08/1995',
            gender: 'Male',
        ...
        if (!useSqlite()) {
            await Kyc.findOneAndUpdate({ user: userId }, {
                status: 'Verified',
...
router.post('/wallet/razorpay/create-order', authenticateToken, async (req, res) => {
...
router.post('/pickups', authenticateToken, async (req, res) => {
    try {
        ...
        if (global.io) {
            global.io.emit('NEW_PICKUP', mockPickup);
        }
...
const handleGetWasteTypes = async (req, res) => {
    try {
        if (!useSqlite()) {
