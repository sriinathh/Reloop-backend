...
const oldStr = `    // Mock Order Creation (Mocking Razorpay interaction to support Expo Go without native crash)
...
const newStr = `    let razorpayOrderId = 'order_mock_' + Math.random().toString(36).substring(7);
    if (!useSqlite()) {
      try {
...
const oldVerifyStr = `    let userEmail = '';
...
const newVerifyStr = `    let userEmail = '';
    if (!useSqlite() && process.env.RAZORPAY_KEY_SECRET) {
      ...
      if (sig !== razorpay_signature) return res.status(400).json({ success: false, message: 'Invalid payment signature' });
    }
    let userName = 'ReLoop User';`;

code = code.replace(oldVerifyStr, newVerifyStr);
